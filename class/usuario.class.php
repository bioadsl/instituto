...
class usuario extends db
{
    ...
    public function cadastrar($dados = array())
    {
        ...
        $q = $this->fetch("SELECT * FROM usr WHERE email = '$email'"); // Verifica se o usuário já está cadastado
        if (!$q) {
            $q = $this->query("INSERT INTO usr VALUES('', '$nome', '$sobrenome', '$cidade', '$estado', '$cep', '$telefone', '$email', '$senha', '$dateTime')");
            if ($q) {
                $erro['sucesso'] = true;
                return $erro;
            } else {
    ...
    public function editar($dados = array())
    {
        ...
        $q = $this->fetch("SELECT * FROM usr WHERE email = '$email' AND idusuario <> $idusuario"); // Verifica se o email já existe
        if (!$q) {
            $q = $this->query("UPDATE usr SET nome = '$nome', sobrenome = '$sobrenome', cidade = '$cidade', estado = '$estado', cep = '$cep', telefone = '$telefone', email = '$email', senha = '$senha' WHERE idusuario = $idusuario");
            if ($q) {
                $erro['sucesso'] = true;
                return $erro;
            } else {
    ...
    {
        $q = $this->query("DELETE FROM usr WHERE idusuario = " . (int) $id);
        if ($q)
            return true;
        else
            return false;
    ...
    public function validaLogin($dados = array())
    {
        ...
        $senha = sha1($senha);
        $q = $this->fetch("SELECT * FROM usr WHERE email = '$email' AND senha = '$senha'");
        if ($q) {
            session_start();
    ...
    {
        $dados = $this->fetch("SELECT * FROM usr WHERE idusuario = " . (int) $id);
        if ($dados)
            return $dados[0];
        else
            return null;
    ...
    {
        $dados = $this->fetch("SELECT * FROM usr");
        if ($dados)
            return $dados;
        else
            return null;
    ...
    public function todosPedidosInfo()
    {
        $dados = $this->fetch("SELECT * FROM tb_pedido");
        if ($dados)
            return $dados;
        else
            return null;
    ...
    public function todasRemessasInfo()
    {
        $dados = $this->fetch("SELECT  distinct remessa FROM tb_pedido");
        if ($dados)
            return $dados;
        else
            return null;
    ...
    public function remessaInfo($remessa)
    {
        $dados = $this->fetch("SELECT  distinct remessa FROM tb_pedido where remessa = " . (int) $remessa);
        if ($dados)
            return $dados;
        else
            return null;
    ...
    public function deletarPedidos($id_pedido)
    {
        $q = $this->query("DELETE FROM tb_pedido WHERE id_pedido = " . (int) $id_pedido);
        if ($q)
            return true;
        else
            return false;
