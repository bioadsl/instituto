...
if (empty($termo)) :
	...
	$sql = 'SELECT distinct id, foto, carteirinha, pl_saude, nome, sexo, dt_nascimento, email, tp_sanguineo, ft_rh, peso, altura, pai, mae, telefone, celular, endereco, alergia, coracao, respiracao, especial, outros, observacao, proximo, tel_proximo, cel_proximo, adesao, status  FROM tab_bravos order by 5';
	...
	$conexao = conexao::getInstance();
	$sql = 'SELECT distinct count(nome) as total_registros from tab_bravos';
	$stm = $conexao->prepare($sql);
	$stm->execute();
	$stm->bindValue(':total_registros', $total_registros);
	...
	$conexao = conexao::getInstance();
	$sql = 'SELECT id, foto, carteirinha, pl_saude, nome, sexo, dt_nascimento, email, tp_sanguineo, ft_rh, peso, altura, pai, mae, telefone, celular, endereco, alergia, coracao, respiracao, especial, outros, observacao, proximo, tel_proximo, cel_proximo, adesao FROM tab_bravos WHERE nome LIKE :nome OR email LIKE :email OR endereco LIKE :endereco OR status LIKE :status';
	$stm = $conexao->prepare($sql);
					...
					<tbody>
						<tr>
							...
							<?php
								$mae = $cliente->mae;								
								$conexao = conexao::getInstance();
								$sql = "SELECT count(nome) as num_filho from tab_bravos where mae = '$mae'";
								$stm = $conexao->prepare($sql);
								$stm->execute();
