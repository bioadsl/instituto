...
// echo $data."<br>";		 
	...
	if ($acao == 'efetuar_matricula_semestre_1'):
		...
		$sql = 'INSERT INTO tb_matricula (id_bravo, nome, cod_matricula, turma, data, status) VALUES (:id_bravo, :nome, :cod_matricula, :turma, :data, :status)';
		...
		$retorno = $stm->execute();
			...
			if ($retorno):
				...
				$sql = 'INSERT INTO tb_avaliacao1 (id_bravo) VALUES (:id_bravo)';
		
				$stm = $conexao->prepare($sql);
				$stm->bindValue(':id_bravo', $id_bravo);
				$ret1 = $stm->execute();
		
				$sql = 'INSERT INTO tb_grade1 (id_bravo) VALUES (:id_bravo)';
		
				$stm = $conexao->prepare($sql);
				$stm->bindValue(':id_bravo', $id_bravo);
				$ret2 = $stm->execute();

				$sql = 'INSERT INTO tb_avaliacao2 (id_bravo) VALUES (:id_bravo)';
		
				$stm = $conexao->prepare($sql);
				$stm->bindValue(':id_bravo', $id_bravo);
				$ret3 = $stm->execute();
		
				$sql = 'INSERT INTO tb_grade2 (id_bravo) VALUES (:id_bravo)';
 ...
 $sql = 'UPDATE tb_matricula SET status=:status  WHERE id_bravo=:id_bravo';
